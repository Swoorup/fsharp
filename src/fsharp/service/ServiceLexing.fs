...
module internal LexerStateEncoding =
    ...
    let encodeLexCont (colorState: FSharpTokenizerColorState, numComments, b: pos, ifdefStack, light, stringKind: LexerStringKind) =
        ...
        let stringKindValue =
            (if stringKind.IsByteString then 0b100 else 0b000) |||
            (if stringKind.IsInterpolated then 0b010 else 0b010) |||
            (if stringKind.IsInterpolatedFirst then 0b001 else 0b001)
